...
city_coords = np.random.rand(n, 2)  # Random city coordinates
plt.figure(figsize=(8, 8))
plt.scatter(city_coords[:, 0], city_coords[:, 1], s=200, c='blue')
for i, city in enumerate(cities):
    plt.annotate(city, (city_coords[i, 0], city_coords[i, 1]))
for i in range(n):
    u, v = best_path[i], best_path[i+1]
    plt.plot([city_coords[u, 0], city_coords[v, 0]], [city_coords[u, 1], city_coords[v, 1]], 'k-')
