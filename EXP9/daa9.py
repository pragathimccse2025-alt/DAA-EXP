...
for i, (bins, label) in enumerate([(ff_bins, 'FF'), (ffd_bins, 'FFD'), (bfd_bins, 'BFD')]):
    for j, b in enumerate(bins):
        axs[i].barh(j, sum(b), color='blue')
        axs[i].set_title(label)
        axs[i].set_xlabel('Used Capacity')
        axs[i].set_ylabel('Bin Index')
        axs[i].set_yticks(range(len(bins)))
        axs[i].set_yticklabels(range(1, len(bins)+1))
plt.tight_layout()
plt.show()